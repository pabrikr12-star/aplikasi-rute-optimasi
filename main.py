...
with st.sidebar:
    ...
    st.success("🔑 Lisensi Gurobi Trial (2849735) Aktif")
...
with col1:
    with st.container(border=True):
        ...
        t_pabrik = st.number_input(
            "Waktu Muat di Pabrik (t_pabrik)", min_value=0, value=10
        )
        # DIUBAH: kata toko -> retailer
        t_retailer = st.number_input(
            "Waktu Bongkar di Retailer (t_retailer)", min_value=0, value=10
        )
        T_max = st.number_input(
            "Batas Kerja Maksimal (T_max)", min_value=60, value=450
        )

with col2:
    with st.container(border=True):
        st.markdown("#### 📦 Kapasitas Armada (Keranjang)")
        cap_mobil1 = st.number_input(
            "Kapasitas Kendaraan 1", min_value=1, value=25
        )
        cap_mobil2 = st.number_input(
            "Kapasitas Kendaraan 2", min_value=1, value=25
        )
...
with col3:
    with st.container(border=True):
        ...
        time_limit = st.number_input(
            "Batas Waktu Komputasi (detik)", min_value=5, value=30
        )
        M_big = st.number_input(
            "Nilai Konstanta M (Big M)", min_value=1000, value=10000
        )

st.divider()

st.markdown("### 🎯 Jumlah Demand Retailer (Retailer 1 - 20)")
st.caption(
    "💡 Masukkan jumlah dalam satuan **Pieces (Pcs)**. Sistem otomatis membaginya dengan 50 dan membulatkan ke atas menjadi satuan **Keranjang** untuk MILP."
)
...
df_hasil_keranjang = pd.DataFrame(
    [demand_converted], columns=[f"R{i}" for i in range(1, 21)]
)
st.dataframe(df_hasil_keranjang, hide_index=True)

st.write(
    f"Total Muatan Hari Ini: **{total_demand} keranjang** | Total Kapasitas Mobil: **{cap_mobil1 + cap_mobil2} keranjang**"
)
...
st.caption(
    "🔒 Matriks waktu antar Retailer telah ditetapkan secara permanen (Pabrik / R0 = 0 Menit)."
)
...
if st.button("🚀 PROSES OPTIMALISASI RUTE PABRIK", type="primary"):
    ...
    with st.spinner("Sedang menjalankan kalkulasi Gurobi..."):
        try:
            model = gp.Model("MTVRP_Direct_Dashboard")

            model.setParam("TimeLimit", time_limit)
            model.setParam("OutputFlag", 0)

            x = model.addVars(
                [(i, j, k) for i in V for j in V if i != j for k in K],
                vtype=GRB.BINARY,
                name="x",
            )
            x_refill = model.addVars(
                [(i, j, k) for i in V for j in V if i != j for k in K],
                vtype=GRB.BINARY,
                name="x_refill",
            )
            ...
            model.setObjective(
                gp.quicksum(
                    (t_input[i, j] + t_retailer) * x[i, j, k]
                    for i in V
                    for j in V
                    if i != j
                    for k in K
                ...
                + gp.quicksum(
                    (t_input[i, 0] + t_pabrik + t_input[0, j] + t_retailer)
                    * x_refill[i, j, k]
                    for i in V
                    for j in V
                    if i != j
                ...
                + gp.quicksum(
                    (t_pabrik + t_input[0, i]) * start[i, k]
                    for i in V
                    for k in K
                )
            ...
            model.addConstrs(
                gp.quicksum(
                    x[i, j, k] + x_refill[i, j, k] for i in V if i != j for k in K
                )
                + gp.quicksum(start[j, k] for k in K)
                == 1
                for j in V
            )
            model.addConstrs(
                gp.quicksum(
                    x[i, j, k] + x_refill[i, j, k] for i in V if i != j
                )
                + start[j, k]
                == gp.quicksum(
                    x[j, i, k] + x_refill[j, i, k] for i in V if i != j
                )
                + end[j, k]
                for j in V
                for k in K
            ...
            model.addConstrs(
                Y[i, k] <= Q[k] - current_demand[i] + M * (1 - start[i, k])
                for i in V
                for k in K
            )
            model.addConstrs(
            ...
            model.addConstrs(
                W[i, k] >= t_pabrik + t_input[0, i] - M * (1 - start[i, k])
                for i in V
                for k in K
            )
            model.addConstrs(
                ...
                + t_retailer
                + t_input[i, j] * x[i, j, k]
                + (t_input[i, 0] + t_pabrik + t_input[0, j])
                * x_refill[i, j, k]
                - M * (1 - x[i, j, k] - x_refill[i, j, k])
                for i in V
                for j in V
...
if "optimization_result" in st.session_state:
    ...
    # DIUBAH: Menampilkan total waktu secara langsung dalam satuan Menit
    total_waktu_menit_asli = res["obj_val"]
    st.metric(
        label="Total Waktu Operasional Armada",
        value=f"{total_waktu_menit_asli} Menit",
    )
    ...
    for k in K:
        ...
        if start_node is not None:
            ...
            active_x = {
                i: j
                for (i, j, m), val in res["x"].items()
                if m == k and val > 0.5
            }
            active_refill = {
                i: j
                for (i, j, m), val in res["x_refill"].items()
                if m == k and val > 0.5
            }
            active_end = {
                i for (i, m), val in res["end"].items() if m == k and val > 0.5
            }
            ...
            st.info(
                f"**Rute Kendaraan {k} (Kapasitas {Q[k]} Keranjang):**  \n{route_text}"
            )
        ...
        if len(seq) > 2:
            ...
            for idx, node in enumerate(seq):
                if node == 0:
                    ...
                    # DIUBAH: kata toko -> Retailer
                    visit_labels.append(f"Retailer R{node}\n(Urutan-{idx})")
            ...
            ax.set_title(
                f"🚚 ALUR DIAGRAM DISTRIBUSI KENDARAAN {k}\n(Kapasitas: {Q[k]} Keranjang)",
                fontsize=13,
                fontweight="bold",
                pad=15,
            )
            ...
            ax.text(
                5,
                -2,
                "KENDARAAN TIDAK BEROPERASI (NON-AKTIF)",
                fontsize=12,
                color="gray",
                ha="center",
                fontweight="bold",
